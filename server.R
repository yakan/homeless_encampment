...
server <- function(input,output,session){
  ...
  output$plot<- renderPlot({
    ...
    if(input$desc_select=="311 Calls"){
      ...
      if(input$category == "a_req"){
        ...
        ggplot(data = whichData() %>%
          ...
          geom_text(aes(label = count),color="#7fcdbb", position = position_stack(vjust = .5),size=5,fontface = "bold") 
        ...
        ggplot(data = whichData() %>%
          ...
          geom_text(aes(label = round(mean,0)),color="#7fcdbb", position = position_stack(vjust = .5),size=5,fontface = "bold")
        ...
        ggplot(data = whichData(), aes(x = factor(MonthCreated))) +
          ...
          theme(axis.text.x = element_text(size = 16),
                ...
                panel.background = element_rect(fill="#e0ecf4"),
                panel.grid = element_line(size = .1))+
          geom_text(stat='count',aes(label = ..count..),color="#7fcdbb", position = position_stack(vjust = .5),size=5,fontface = "bold")
    ...
    }else if(input$desc_select=="Crime"){
      ...
      #second plot
      else if(input$desc_crime_var=="Week"){
        ggplot(data1(),aes(x=wday(DateOccurred,label=T,abbr=T)))+
          ...
          theme(axis.text.x = element_text(angle = 30, hjust = 1, size = 12),
                ...
                panel.grid = element_line(size = .1))+
          geom_text(stat='count',aes(label = ..count..),color="#7fcdbb", position = position_stack(vjust = .5),size=5,fontface = "bold")
      }else if(input$desc_crime_var=="Age"){
        ...
        ggplot(data3,aes(x=VictimAge))+
          geom_histogram(binwidth=10,fill="#2c7fb8",color="black")+
          ...
          theme(axis.text.x = element_text(angle = 30, hjust = 1, size = 12),
                ...
                panel.background = element_rect(fill="#e0ecf4"),
                panel.grid = element_line(size = .1))
      }else if(input$desc_crime_var=="Race"){
        #data
        data4=data1() %>%
      ...
      data1 %>%
        ...
        geom_text(size = 9,color="#7fcdbb" ,position = position_stack(vjust = 0.5),fontface = "bold")+
        scale_fill_manual(name="",values=c("#2c7fb8","#fc4e2a"))+
        theme(axis.text.x = element_text(angle = 30, hjust = 1, size = 12),
              axis.text.y = element_text(size = 16),
              axis.title = element_text(size=16),
              plot.title = element_text(hjust = 0.5),
  ...
  output$plot_reg = renderPlot({
    ...
    ggplot(variable(), aes(x = choice1, y = choice2))+ 
      ...
      theme_economist()+theme(axis.text = element_text(size = 13),
  ...
  output$bar=renderPlot({
    plot_data() %>%
      ...
      geom_text(size = 4,color="#7fcdbb" ,position = position_stack(vjust = .5),fontface = "bold")+
      theme(panel.background = element_rect(fill="#e0ecf4"),
            panel.grid = element_line(size = .1))
  })
